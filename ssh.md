...
```
Далее, для того чтобы использовать сгенерируемый ключ в ssh , требуется его передать хосту, к которому вы будете подключаться. Передаваться должен .pub, приватный ключ должен оставаться только у пользователя
```bash
[root@localhost .ssh]# ssh-copy-id -i местоположение открытого ключа root@ip-устроства к которому подключаетесь
```
